...
class QuittungstonHMSecSirWM extends IPSModule
{
    // Helper
    use QTHMSECSIRWM_backupRestore;
    use QTHMSECSIRWM_muteMode;
    use QTHMSECSIRWM_toneAcknowledgement;
    use QTHMSECSIRWM_triggerVariable;
    ...
    public function Create()
    {
        ...
        // Variables
        // Tone acknowledgement
        $profile = 'QTHMSECSIRWM.' . $this->InstanceID . '.AcousticSignal';
        ...
        // Mute mode
        $profile = 'QTHMSECSIRWM.' . $this->InstanceID . '.MuteMode.Reversed';
        if (!IPS_VariableProfileExists($profile)) {
        ...
        // Timers
        $this->RegisterTimer('StartMuteMode', 0, 'QTHMSECSIRWM_StartMuteMode(' . $this->InstanceID . ');');
        $this->RegisterTimer('StopMuteMode', 0, 'QTHMSECSIRWM_StopMuteMode(' . $this->InstanceID . ',);');
    }
    ...
    public function Destroy()
    {
        ...
        if (!empty($profiles)) {
            foreach ($profiles as $profile) {
                $profileName = 'QTHMSECSIRWM.' . $this->InstanceID . '.' . $profile;
    ...
    public function MessageSink($TimeStamp, $SenderID, $Message, $Data)
    {
        ...
        switch ($Message) {
            ...
            case VM_UPDATE:
                ...
                $scriptText = 'QTHMSECSIRWM_CheckTriggerVariable(' . $this->InstanceID . ', ' . $SenderID . ', ' . $valueChanged . ');';
    ...
    public function GetConfigurationForm()
    {
        ...
        $formData['elements'][1]['items'][0] = [
            ...
            'items' => [$formData['elements'][1]['items'][0]['items'][0] = [
                'type'    => 'SelectInstance',
                'name'    => 'AlarmSiren',
                'caption' => 'HM-Sec-Sir-WM Instanz, Kanal 4',
                'width'   => '600px',
            ],
        ...
        $formData['status'][0] = [
            'code'    => 101,
            'icon'    => 'active',
            'caption' => 'Quittungston HM-Sec-Sir-WM wird erstellt',
        ];
        $formData['status'][1] = [
            'code'    => 102,
            'icon'    => 'active',
            'caption' => 'Quittungston HM-Sec-Sir-WM ist aktiv (ID ' . $this->InstanceID . ')',
        ];
        $formData['status'][2] = [
            'code'    => 103,
            'icon'    => 'active',
            'caption' => 'Quittungston HM-Sec-Sir-WM wird gelöscht (ID ' . $this->InstanceID . ')',
        ];
        $formData['status'][3] = [
            'code'    => 104,
            'icon'    => 'inactive',
            'caption' => 'Quittungston HM-Sec-Sir-WM ist inaktiv (ID ' . $this->InstanceID . ')',
        ];
        $formData['status'][4] = [
            'code'    => 200,
            'icon'    => 'inactive',
